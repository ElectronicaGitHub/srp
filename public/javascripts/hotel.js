$(function () {
	...
	$('#picker').dateRangePicker({
		...
		console.log(dateDiffInDays(obj.date1, obj.date2) + 1);
		a = moment(obj.date2).locale('ru').format('D MMMM YYYY');
		console.log(a);
		days = dateDiffInDays(obj.date1, obj.date2) + 1;
		price = hotel.price[0];
		if (days > 3) price = hotel.price[1];
		if (days > 4 && days <= 6 ) price = hotel.price[2];
		if (days > 6) price = hotel.price[3];
